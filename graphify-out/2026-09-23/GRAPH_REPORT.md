...
Cohesion: 0.10
Nodes (14): BodyRegionSettings, _default_profiles(), Enum, str, How much of each kind of mark one region gets, as multipliers of the sliders., Where skin marks tend to fall on a body, as a starting point., Where the regions come from, and what each does to the marks., One region's multiplier for ``effect`` after another, in :data:`REGIONS` order. (+6 more)
